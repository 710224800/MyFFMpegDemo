...
int _create_file_inner(MuxerData *handle,
                       const char *path,
                       jint vfps,
                       jint videoCode,
                       jint vwidth,
...
Java_com_decoder_xiaomi_Mp4Muxer_nativeInit(JNIEnv *env, jclass type) {

    jclass localClass = (*env)->FindClass(env,
                                          "com/decoder/xiaomi/Mp4Muxer");
    mp4MuxerField = (*env)->GetFieldID(env, localClass, "nativeId", "I");
    (*env)->DeleteLocalRef(env, localClass);
}


JNIEXPORT jint JNICALL
Java_com_decoder_xiaomi_Mp4Muxer_initMuxer(JNIEnv *env, jobject instance, jstring outPath_,
                                           jint vFps,
                                           jint videoCode,
    ...
    MuxerData *decoder = (MuxerData *) (*env)->GetIntField(env, instance, mp4MuxerField);
    if (decoder != NULL) {
        (*env)->ReleaseStringUTFChars(env, outPath_, outPath);
        return -1;
    }
    decoder = (MuxerData *) malloc(sizeof(MuxerData));
    if (decoder == NULL) {
    ...
    (*env)->SetIntField(env, instance, mp4MuxerField, (int) decoder);
...
JNIEXPORT jint JNICALL
Java_com_decoder_xiaomi_Mp4Muxer_writeAudio(JNIEnv *env, jobject instance, jbyteArray data_,
                                            jint dataLen) {
...
JNIEXPORT jint JNICALL
Java_com_decoder_xiaomi_Mp4Muxer_writeVideo(JNIEnv *env, jobject instance, jbyteArray data_,
    ...
    int ret = _write_video_frame_inner(decoder, (unsigned char *) data, dataLen, durtion,
                                       iskeyFrame);
    (*env)->ReleaseByteArrayElements(env, data_, data, 0);
    return ret;
}
...
JNIEXPORT jint JNICALL
Java_com_decoder_xiaomi_Mp4Muxer_closeFile(JNIEnv *env, jobject instance) {
    MuxerData *decoder = (MuxerData *) (*env)->GetIntField(env, instance, mp4MuxerField);
    if (decoder == NULL) {
