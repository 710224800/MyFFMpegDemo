...
Java_com_decoder_xiaomi_Mp4Read_nativeInit(JNIEnv *env, jclass type) {
    jclass localClass = (*env)->FindClass(env,
                                          "com/decoder/xiaomi/Mp4Read");
    mp4ReadField = (*env)->GetFieldID(env, localClass, "nativeId", "I");
    (*env)->DeleteLocalRef(env, localClass);
    LOGE("nativeInit ");
}

JNIEXPORT void JNICALL
Java_com_decoder_xiaomi_Mp4Read_init(JNIEnv *env, jobject instance) {
    av_register_all();
    Mp4info *mp4info = (Mp4info *) (*env)->GetIntField(env, instance, mp4ReadField);
    if (mp4info != NULL) {
    ...
    memset(mp4info, 0, sizeof(Mp4info));
    (*env)->SetIntField(env, instance, mp4ReadField, (int) mp4info);
}
...
Java_com_decoder_xiaomi_Mp4Read_openFile(JNIEnv *env, jobject instance, jstring path_,
...
JNIEXPORT jint JNICALL
Java_com_decoder_xiaomi_Mp4Read_getVideoFrame(JNIEnv *env, jobject instance, jbyteArray data_,
...
JNIEXPORT jint JNICALL
Java_com_decoder_xiaomi_Mp4Read_closeFile(JNIEnv *env, jobject instance) {
    Mp4info *mp4info = (Mp4info *) (*env)->GetIntField(env, instance, mp4ReadField);
    if (mp4info == NULL) {
        return -1;
    }
